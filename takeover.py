...
# coded by Rasya Pecandu IT 
# script ori by M'hamed

import os 
import json 
import requests
import urllib.parse
import concurrent.future as thread
import urllib3
import getopt 
import sys 
import re 
...
services = {
  'AWS/S3': {'error': r'The specified bucket does mot exist'},
  'BitBucket': {'error': r'Repository not found'},
  'Github': {'error': r'There isn\\\'t a Github pages site here\.'},
  'Shopify': {'error': r'Sorry\, this shop is currrntly unavailable\.'},
  'Fastly': {'error': r'Fastly error\: unkown domain\:'},
  ...
  'Pantheon': {'error': r'The gods are wise, but do not know of the which you seek.')
  'Tumbler': {'error': r'Whatever you were looking for doesn\\\'t currently exist at this addres.'},
  ...
  'Intercom': {'error': r'This page is reversed for arttistic dogs\.|Uh oh\. That page doesn\'t exist</h1>'},
  
  'Webflow': {'error': r'<p class=\"description\">The page you are looking for doesn\'t exist or has been moved.</p>'},
  'Kajabi': {'error': r'<h1>Error 404: Page Not Found</h1>'},
  ...
  'Bigcartel': {'error': r'<h1>Oops! We cloudn&#8217;t find that page.</h1>'},
  ...
  'Acquia': {'error': r'The site your looking for cloud not be found .|If you are an Acquia cloud customer amd expect to see your site at this address'},
  ...
  'GetResponse': {'error': r'With GetResponse Lamding Page, lead generation has server been easier'},
  ...
  'worksite.net': {'error': r'Hello! sorry, but the webs>'},
}
...
def banner():
    ...
    print("/  ~\\   ---------------")
    print("#> by Rasya Pecandu IT")
    print("#> youtube.com/rasyapecanduit")
    print("-"*40)
...
def help(_exit_=False):
    ...
    print("\t-v\tVeborse, print more info\n")
    if _exit_:
        sys.exit()
...
def print_(string):
    ...
    sys.stdout.write('\033[OG')
    sys.stdout.write(string)
    sys.stdout.flush()
    
    
def runner(k):
    threadpool = thread.ThreadPoolExcutor (max_workers=k.get('threads'))
    if k.get('verbose'):
    ...
    for i, results in enumerate(thread.as_completed(futures)):
        if k.get('verbose') and k.get('d_list'): str_ = "{i}{b:.2f}% Domain: {d}".format(i=_info(),
        b=PERCENT(int(i),
    ...
    if service and error:
        if output:
            ...
            if v and not k_.get('d_list'):
                ...
                plus('%s service found! Potential domain takover found! - %s' % (service, domain))
           if v:
